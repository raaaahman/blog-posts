...
For this reason, every new feature should be an opportunity to question the actual design of this code base. There's two benefits from a better design that could satisfy both the _Product Owner/customer_ and the developers team:

- A suited design may allow for the feature to be **implemented faster**
...
... we actually can (for specific cases)! Experienced programmers have noticed anti-patterns that have been repeated through numerous software; anti-patterns that usually lead to bugs in a more a less expected manner. A great deal of these anti-patterns have been compiled by Martin Fowler and Kent Beck in a book named _Refactoring: Improving the Design of Existing Code_[^3], under the term of **code smells**.

[^3]: Martin Fowler. _Refactoring: Improving the Design of Existing Code, 2nd Edition_. Addison-Wesley Professional, 2018.

The following example comes from the _Bug Zero Kata_[^4] by Johan Martinsson. This _code kata_ aims to prevent bugs happening with the implementation of new features by refactoring an application close to a Trivial Pursuit game.

[^4]: GitHub. "BugsZero Kata". Last modified March 25, 2020. [https://github.com/martinsson/BugsZero-Kata](https://github.com/martinsson/BugsZero-Kata)
...
![Data clumps, when your data should REALLY stick together!]()
...
Let's take the data clump sniffed earlier as an example. The book recommends a refactoring named _Extract Class_: we'd rather have all the related data bound together in instances that can represent each individual player. So if a player leaves, he takes all his coins with him.
...
Each one of these functions are simple _getters and setters_: they **encapsulate** the access to the current player's data (position on the board, coins in purse, if she is in penalty box...), but the arrays still exist individually! What I can do now is replacing each call to these arrays to a call to the adequate function instead, one by one, testing between each replacement that I didn't break anything. With a smart <abbr title="Integrated Development Environment>IDE</abbr> or a little search & replace, I can also speed up the process by a lot.

![It is always safer to bind the related data together.]()
...
I could have stopped here, but these getters and setters only delegate to the `Player` getters and setters... and that's a bit too much of **indirection** for my liking. So I did a little clean up by:

- Extracting the access to the current player instance in a `getCurrentPlayer()` function that should be more explicit.
- Inlining all the getters and setters at the game level, which means that I replaced each of these function calls by the code that is inside these functions
...
[^8]: If you want a follow through my whole process, you can explore the [pull request](https://github.com/martinsson/BugsZero-Kata/pull/30) I made for it on GitHub.

Knowing the specific refactorings helps a lot in defining the steps to take (but I won't lie, I had the book on my desk during the whole kata). And the smaller of them are built to be done by most of the recent IDEs on a simple shortcut, which speeds up the process by a whole lot while making it risk free!
...
- You have **avoided some bugs** before anyone (QA or end customer) have stumbled upon them
- With a more **understandable code base**, the eventual other bugs will still be easier to track and fix
- **Refactoring is a skill**, as much as implementing features, and each time you do it, you get better and faster at it!
